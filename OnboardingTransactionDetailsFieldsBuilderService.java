...
public class NbdPremiumThresholdRisk {
    ...
    public static CaseRisk premiumThreshold(
            final ScreenDescription screenDescription,
            final Map<String, List<String>> overallCaseRisk) {

        CaseRisk forcedValue = CaseRisk.CASE_RISK_STANDARD;
        ...
        if (!"BE".equals(businessOrigin)) {
            ...
            forcedValue = evaluateBePremiumThreshold(
                    transactionAmount,
                    bePoliciesAmount,
                    contractType,
                    overallCaseRisk
    ...
    private static CaseRisk evaluateNonBePremiumThreshold(
            final BigDecimal transactionAmount,
            final BigDecimal policiesAmount,
            final Map<String, List<String>> overallCaseRisk) {
    ...
    private static CaseRisk evaluateBePremiumThreshold(
            final BigDecimal transactionAmount,
            final BigDecimal bePoliciesAmount,
        ...
        if (sum.compareTo(thresholdConfig.maxSlice()) >= 0
            ...
            o(overallCaseRisk, HIGH, thresholdConfig.reason());
            return CaseRisk.CASE_RISK_HIGH;
        }
    ...
    private static boolean isSecondSliceThresholdReached(
            final BigDecimal transactionAmount,
            final BigDecimal policiesAmount,
            final BigDecimal secondSlice) {

        return transactionAmount
                .add(policiesAmount.remainder(secondSlice))
                .divide(secondSlice)
                .compareTo(BigDecimal.ONE) >= 0;
    }

    private static boolean isFirstSliceThresholdReached(
            final BigDecimal transactionAmount,
            final BigDecimal policiesAmount,
            final BigDecimal firstSlice) {

        return transactionAmount.compareTo(firstSlice) >= 0
                || transactionAmount.compareTo(firstSlice) < 0
                && transactionAmount.add(policiesAmount).compareTo(firstSlice) >= 0;
    }
    ...
    private static BigDecimal getBigDecimal(
        ...
        return ChecklistUtils.getFieldById(screenDescription, fieldId)
    ...
    private static void addRiskReason(
            final Map<String, List<String>> overallCaseRisk,
            final String risk,
            final String reason) {
    ...
    private record ThresholdConfig(
            BigDecimal firstSlice,
            BigDecimal secondSlice,
            BigDecimal maxSlice,
            String reason) {
    }
}
